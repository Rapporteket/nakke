...
RegData <- NakkeHentRegData(datoFra = datoFra)  #NakkeRegDataSQL(datoFra = datoFra)
RegData <- NakkePreprosess(RegData = RegData)
RegData24mnd <- RegData[RegData$OprDato > as.POSIXlt(datoFra24mnd), ]
RegData12mnd <- RegData[RegData$OprDato > as.POSIXlt(datoFra12mnd), ]
AarNaa <- as.numeric(format(Sys.Date(), "%Y"))
# Nye variabler:
RegData$Mnd <- RegData$MndNum

shtxt <- ifelse(reshID == 0, 'Hele landet',
                as.character(RegData$SykehusNavn[match(reshID, RegData$ReshId)]))
tidsenhRapp <- 'Halvaar'
skalShFig <- 0.32
@
...
tabAvdSiste12mnd <-  tabAntOpphShMnd(RegData12mnd, antMnd=12)
names(tabAvdSiste12mnd) <- substr(names(tabAvdSiste12mnd), 1, 3)
print(xtable::xtable(tabAvdSiste12mnd,
                     digits=0,
...
tabUsiste3mnd <- tabAntSkjema(RegData12mnd, skjemastatus=1, datoTil = as.Date(dato)-100)
tab <- tabAntSkjema(RegData12mnd, skjemastatus=1)[ ,1:3] # Mister 2 legeskjema?
tab <- cbind(
  tab[ , 1:2],
  'Oppf. 3 mnd.' = tabUsiste3mnd[,3],
  'Oppf. 3 mnd. (%)' = 100*tabUsiste3mnd[ ,3]/tabUsiste3mnd[,1]
)

xtable::xtable(tab, align=c('l', rep('r', 4)),
               digits=c(0,0,0,0,1),
               caption = paste0('Antall ferdigstilte skjema av hver type per måned og avdeling,
               for operasjoner utført siste 12 måneder.
               For lege- og pasientskjema er registreringer fram til dagens dato med,
               mens for oppfølgingsskjema er antall og andel basert på operasjoner i perioden ',
               datoFra12mnd,' til ', (as.Date(dato)-100), '.'))
...
NakkeFigGjsnTid(RegData=RegData, preprosess=FALSE, valgtVar='NDIendr3mnd',
           ...
           reshID=reshID, enhetsUtvalg=1, outfile='NDIendr3mndFremUmTid.pdf')
...
NakkeFigGjsnTid(RegData=RegData, preprosess=FALSE, valgtVar='NRSsmerteArmEndr3mnd',
           fremBak=1, myelopati=0, tidsenhet=tidsenhRapp,
           reshID=reshID, enhetsUtvalg=1, outfile='NRSsmerteArmEndr3mndBakMmTid.pdf')
...
NakkeFigGjsnTid(RegData=RegData, preprosess=FALSE, valgtVar='NRSsmerteNakkeEndr3mnd',
                tidsenhet=tidsenhRapp,
           reshID=reshID, enhetsUtvalg=1, outfile='NSRsmerteNakkeEndr3mndTid.pdf')
